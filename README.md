...
| `keyVars` | 关键变量词表（**新闻标题中命中即归"关键变量"分类 + 标题里加粗金棕高亮**）。白酒=批价/库存/动销；港股按公司经营核心指标自配 |
| `moat` | 护城河一句话（总览卡片显示，可选） |
| `thesis` | 投资逻辑/持有理由（总览卡片显示，可选） |
| `targetPrice` | 你的目标价（总览卡片显示，可选） |

> 💡 **moat / thesis / targetPrice 是给你自己看的"投资备忘"**：前端总览（Dashboard）页会为每家公司渲染一张基本面卡，把这三项列出来。你不用写很长的论文——一句话点出"为什么持有它"就够了，比如 `"moat": "品牌+定价权"`, `"thesis": "PE 跌到 20 以下加仓"`, `"targetPrice": "1700"`。
...
**A. 爬虫数据质量（crawler/crawl.mjs）**
- **相关性过滤**：标题必须命中公司名/别名（`ALIASES` 表）才保留，过滤掉"顺带提到公司名"的大盘/ETF/行业泛闻等噪音。
- **标题去重**：同一新闻多源转载只留一条（按归一化标题判重）。
- **分类词表扩充**：`CAT_RULES` 由 4 类扩到 业绩/公告/股价/行业/产品 5 类，命中关键词即归类，降低"其他"占比。
- **单家封顶**：每家公司最多 60 条，避免一家刷屏。

**B. 前端交互（index.html / app.js / styles.css）**
- **时间筛选**：新闻视图新增「今天 / 近3天 / 近7天 / 全部」筛选，默认**近7天**，缓解长列表在手机端翻不动。
- **隐藏已读**：勾选后自动隐藏已标记"已读"的新闻；统计条新增**未读数**。
- **favicon + 页面描述**：标签页显示 📊 图标，搜索引擎有摘要。
- 配套样式与缓存版本号（`?v=3`）已更新，刷新即生效。
